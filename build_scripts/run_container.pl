...
if ($interactive) {
    $dockerCmd = '/bin/bash';
    $dockerItFlag = '-it'; # for interactive container
}
...
exec("docker", "run", "--rm", "$dockerItFlag", "--init", "--volume", "$topPath:/app", "--publish", "$hostIP:$ports{$componentToRun}:$ports{$componentToRun}", "$imageName", "$dockerCmd");